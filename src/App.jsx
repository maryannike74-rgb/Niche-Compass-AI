...
export default function NicheCompassAI() {
  ...
  const analyzeNiche = async () => {
    ...
    try {
      ...
      // Check if we should use mock (API failed or not configured)
      if (data.useMock || !data.success) {
        console.log('Using mock analysis (API not available)');
  ...
  const generatePremiumMockAnalysis = (nicheIdea) => {
    ...
    // Intelligent niche-specific mock for storytelling niches (folktales, myths, legends, fables)
    const isFolktale = nicheIdea.toLowerCase().includes('folktale') || nicheIdea.toLowerCase().includes('folk tale') || nicheIdea.toLowerCase().includes('folklore');
    const isMythology = nicheIdea.toLowerCase().includes('myth') || nicheIdea.toLowerCase().includes('legend');
    const isFables = nicheIdea.toLowerCase().includes('fable');
    const isStory = isFolktale || isMythology || isFables;
    
    if (isStory) {
      // Strategic story-based content
      const region = isFolktale && nicheIdea.toLowerCase().includes('african') ? 'African' : 
                     isFolktale && nicheIdea.toLowerCase().includes('asian') ? 'Asian' : 
                     isFolktale && nicheIdea.toLowerCase().includes('european') ? 'European' : 'Cultural';
      
      return {
        executiveSummary: `${nicheIdea} represents a compelling Hidden Gem opportunity in the educational storytelling space. The niche benefits from evergreen content potential (stories remain relevant indefinitely), strong cultural identity appeal, and minimal competition from established creators. Primary monetization includes AdSense (educational CPMs $6-12), cultural brand partnerships, and digital story collections. The main challenge is balancing entertainment with educational framing to maximize reach beyond diaspora audiences. Recommended strategy: lead with universal themes (courage, wisdom, consequences) while highlighting cultural specificity as the unique angle.`,
        overallScore: 82,
        verdict: "Hidden Gem",
        scores: { demand: 7, monetization: 8, competition: 4, facelessFit: 10, contentSupply: 10, differentiation: 9, safetyScore: 10 },
        positioning: `Position as the bridge between ${region.toLowerCase()} oral tradition and modern storytelling - making ancient wisdom accessible and relevant to contemporary audiences through psychological and philosophical lenses.`,
        targetAudience: `Primary: Ages 18-45 with cultural heritage connection seeking authentic stories, plus educators (25-50) looking for diverse curriculum content, and philosophy/psychology enthusiasts drawn to timeless wisdom. Secondary: Parents wanting meaningful children's content. All segments value substance over sensationalism and appreciate learning through narrative.`,
        channelFormat: "Animated storytelling with voiceover narration, or illustrated story format with text overlays and atmospheric music",
        contentPillars: ["Story Retelling with Modern Commentary", "Cultural Context and Historical Background", "Universal Lessons and Psychological Insights"],
        videoIdeas: {
          quickWins: [
            "Anansi the Spider: The Trickster Who Outsmarted Death Itself",
            "Why the Tortoise Has a Cracked Shell (And What It Teaches About Greed)",
            "Mami Wata: The African Mermaid Who's Nothing Like Disney",
            "The Talking Skull: A Folktale About Knowing When to Stay Silent",
            "How the Leopard Got His Spots: Pride, Deception, and Permanent Consequences"
          ],
          authorityBuilders: [
            "The Real Meaning Behind Anansi Stories: Resistance Through Trickery",
            "Why ${region} Folktales Always Feature Animals (Anthropomorphism Explained)",
            "The Griot Tradition: How Stories Were Preserved for Thousands of Years",
            "Comparing ${region} Creation Myths to Other World Mythologies",
            "The Psychology of Trickster Figures Across Cultures",
            "Oral Tradition vs Written: Why Folktales Change and Why That Matters",
            "${region} Moral Philosophy Hidden in Children's Stories",
            "The Historical Context That Shaped These Tales",
            "Why Western Adaptations Miss the Point of ${region} Stories",
            "The Art of Call-and-Response in Traditional Storytelling"
          ],
          viralPotential: [
            "I Told My Kids ${region} Folktales Instead of Disney - Here's What Happened",
            "This ${region} Story Predicted Modern Psychology 500 Years Ago",
            "The Darkest ${region} Folktale You've Never Heard (Not for Kids)",
            "${region} Stories Disney Will Never Touch (Too Real)",
            "I Analyzed 50 ${region} Folktales - Every One Had This Dark Pattern",
            "The ${region} Story That Explains Why We Fear the Unknown",
            "This Simple ${region} Fable Contains the Secret to Dealing with Narcissists",
            "Why ${region} Grandmother Stories Are More Brutal Than Grimm's Fairy Tales",
            "The ${region} Tale That Perfectly Explains Toxic Relationships",
            "I Traced This ${region} Story Back 1000 Years - It Changed Completely"
          ],
          evergreen: [
            "Top 20 Essential ${region} Folktales Everyone Should Know",
            "${region} Folktales Explained: A Beginner's Guide",
            "The Complete Guide to ${region} Mythological Creatures",
            "${region} Stories for Kids: Age-Appropriate Tales by Theme",
            "How to Tell ${region} Stories: A Storyteller's Guide"
          ]
        },
      competitorAnalysis: [
